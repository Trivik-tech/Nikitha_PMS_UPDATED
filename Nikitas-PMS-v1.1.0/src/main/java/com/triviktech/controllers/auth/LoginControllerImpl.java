...
/**
 * LoginControllerImpl is a REST controller that implements the LoginController
 * interface.
 *
 * Responsibilities:
 * 1. Handle user login requests by verifying credentials through the
 * LoginService.
 * 2. Generate and return a JWT token upon successful login.
 * 3. Provide an endpoint to retrieve the role of a user based on a provided JWT
 * token.
 *
 * This controller delegates authentication logic to LoginService and
 * token-related
...
public class LoginControllerImpl implements LoginController {
    ...
    @Override
    public ResponseEntity<Map<String, String>> getRole(@PathVariable("token") String token) {
        Map<String, String> response = new HashMap<>();
